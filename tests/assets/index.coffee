...
# ###
# 'This is a key with some #{var}': 'This is [b key] with some #{var} and <i>HTML</i>'
...
#  * @see  Pug language
# ###
# 'test %firstName, %age': 'Test #{firstName_willbe_escaped}, !{age_will_not_be_escaped}'
...
# 	1: '#{users[0].name} has shared'
# 	2: '#{users[0].name} and #{user[1].name} had shared'
# 	else: '#{users[0].name} and #{users.length} others had shared'
	
# 'a folder': i18n.switch 'folders.length', # if some key contains "." use table to describe path, example: i18n.switch ['fold.er', 'length']
# 	0: 'no folder'
# 	1: 'one folder'
# 	2: 'two folders'
# 	else: '#{folders.length} folders'
